...
#======================Falsk(main)======================#
def start_flask_server(config, uwsgi):
...
def check_and_start_server(config, uwsgi=False):
    if not check_config():
    ...
    if not check_muipserver():
        ...
        sys.exit(1)
    start_flask_server(config, uwsgi)


if __name__ == '__main__':
    if len(sys.argv) < 2:
        #raise Exception("请提供命令行参数: server 或 initdb")
        command = "server"
    else:
        command = sys.argv[1]
    config = load_config()
    if command == "server": 
        check_and_start_server(config)
    elif command == "initdb":
        if not check_config():
            print("#=====================[Config]文件校验失败！请检查服务配置=====================#")
            sys.exit(1)
        ...
        raise Exception("未知的操作！必须是以下命令: server 或 initdb")
elif __name__ == 'main':
    config = load_config()
    check_and_start_server(config,True)