            ...
            if not email_exists:
                ...
                for recover_code_info in session["recover_codes"]:
                    if (
                        ...
                        and recover_code_info["reset_code"] == verifycode
                ...
                flash(
                    f"密码长度不能小于 {get_config()['Security']['min_password_len']} 字节",
                    "error",
                )
            else:
...
def recover_code():
    ...
    if "recover_codes" in session:
        ...
        for n in range(len(session["recover_codes"])):
            ...
            if recover_code_info["timeout"] >= datetime.now(utz):
        ...
        if len(not_timeout_li) > 5:
            ...
            except_time = (
                session["recover_codes"][not_timeout_li[-6]]["timeout"]
                .astimezone(ctz)
                .strftime("%Y-%m-%d %H:%M:%S")
            )
            return json_rsp_with_msg(
                repositories.RES_FAIL,
                f"发送验证码频率超过限制，请在{except_time}后再试",
                {},
            )
    if "send_code_timeout" in session and session["send_code_timeout"] > datetime.now(
        ...
        except_time = (
            session["send_code_timeout"].astimezone(ctz).strftime("%Y-%m-%d %H:%M:%S")
        )
        return json_rsp_with_msg(
            repositories.RES_FAIL, f"发送验证码间隔为60秒，请在{except_time}后再试", {}
        )

    reset_code = "".join(random.choices(string.digits, k=get_config()['Security']['verify_code_length']))
    ...
    if "recover_codes" in session:
        for n in range(len(session["recover_codes"])):
            ...
            if recover_code_info["email"] == email:
                ...
                session["recover_codes"][n]["valid"] = False
        session["recover_codes"].append(new_recover_code_info)
    else:
        session["recover_codes"] = [
            recover_code_info,
        ]
    # 设置下次可以发送验证码的时间
